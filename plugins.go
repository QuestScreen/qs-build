	...
	for _, plugin := range plugins {
		for _, module := range plugin.Modules {
			if strings.HasPrefix(plugin.ImportPath, "github.com/QuestScreen/QuestScreen/") {
				...
				if err = executeInspector(plugin.ImportPath, module.Name,
					module.ImportName, &path); err != nil {
					panic(err)
				}
			} else {
				if err := executeInspector(
					plugin.ImportPath, module.Name, module.ImportName, nil); err != nil {
					panic(err)
				}
			}
		}
